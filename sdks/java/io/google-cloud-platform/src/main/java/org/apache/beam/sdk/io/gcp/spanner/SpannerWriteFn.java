...
class SpannerWriteFn extends AbstractSpannerFn<Mutation, Void> {
  ...
  @VisibleForTesting
  SpannerWriteFn(SpannerIO.Write spec) {
    this.spec = spec;
  }
   ...
   *
   * @throws AbortedException if the commit fails or IOException or InterruptedException if backing
   *     off between retries fails.
   */
  private void flushBatch() throws AbortedException, IOException, InterruptedException {
    ...
    while (true) {
      ...
      try {
        ...
        LOG.error("Error writing to Spanner ({}): {}", exception.getCode(), exception.getMessage());
