...
public class OrderedCode {
  ...
  private byte convert(boolean invert, byte val) {
   ...
   * internal encoded byte array store.
   *
   * Port of OrderedCode::WriteNumIncreasing(string*, uint64).
   ...
   * internal encoded byte array store.
   *
   * Port of OrderedCode::WriteNumDecreasing(string*, uint64).
   * Note that the specified long is treated like a uint64, e.g.
   * {@code new OrderedCode().writeNumDecreasing(-1L).getEncodedBytes() &lt;
   ...
   *
   * Port of Bits::Log2Floor64(uint64) except that only non-negative longs are
   * accepted (which seemed more expected than treating negative longs as if
   * they were uint64s).
   */
  @VisibleForTesting
   ...
   * Port of SignedEncodingLength(int64) from ordered-code.cc.
   */
  @VisibleForTesting
  ...
  /**
   * Port of OrderedCode::WriteSignedNumIncreasing(string*, int64).
  ...
  private byte[] readBytes(boolean invert) {
    ...
    while (j < store.length - 1) {
      ...
      if (b == convert(invert, ESCAPE1)) {
        arraycopy(invert, store, copyStart, decodedArray, outIndex,
            j - copyStart - 1);
   ...
   *
   * Port of OrderedCode::ReadSignedNumIncreasing(StringPiece*, int64*).
   *
